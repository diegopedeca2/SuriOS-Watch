            ...
            override fun onScanResult(callbackType: Int, result: ScanResult) {
                ...
                onObservation(
                    BleObservation(
                        ...
                        bluetoothClass = runCatching { result.device.bluetoothClass?.deviceClass }.getOrNull(),
                        deviceAddressType = addressTypeOf(result.device),
...
@SuppressLint("NewApi")
private fun addressTypeOf(device: BluetoothDevice): Int? =
    if (Build.VERSION.SDK_INT >= 35) {
        runCatching { device.addressType }.getOrNull()
    } else {
        null
    }
